-- PotBot — Asset Registry v1
-- Isolated in schema `registry` to avoid colliding with the existing
-- public.pots table used by the live app. All objects are CREATE ... IF NOT
-- EXISTS so this migration is safe to run repeatedly.
--
-- Safety invariants enforced at the DB level (defence in depth, mirrored in
-- @potbot/asset-registry):
--   * mint is NULLABLE — a pending_verification asset must NOT carry a guessed mint.
--   * an asset may only be `enabled` when it is `verified` AND has a `mint`.
--   * pot_assets weights are bounded; the "sum == 10000 bps" rule is enforced
--     in the application layer (cross-row), and re-checked here via a trigger.
...
create table if not exists registry.assets (
  ...
  mint                text unique,                         -- NULL while pending_verification
  ...
  -- Core safety gate: cannot enable a pending/mint-less asset.
  constraint assets_enable_requires_verified_mint
    check (enabled = false or (verified = true and mint is not null))
);
...
-- ─────────────────────────────────────────────────────────────────────────
-- POTS (registry/planning layer — separate from public.pots on-chain mirror)
-- ─────────────────────────────────────────────────────────────────────────
...
-- Enforce "only addable assets in a pot" + per-asset cap at write time.
...
create trigger trg_check_pot_asset
  before insert or update on registry.pot_assets
  for each row execute function registry.check_pot_asset();

-- Enforce max 10 assets + weights summing to 10000 bps for ACTIVE pots.
...
begin
  ...
  select coalesce(sum(target_weight_bps),0), count(*) into total, cnt
    from registry.pot_assets where pot_id = pid;
  if cnt > 10 then raise exception 'pot % exceeds 10 assets', pid; end if;
  select status into st from registry.pots where id = pid;
  -- Only require the exact 10000 sum once a pot is activated; drafts may be partial.
...
create trigger trg_check_pot_weights
  after insert or update or delete on registry.pot_assets
