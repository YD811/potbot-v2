...
const YIELD_OPTIONS = [
  ...
  { value: 5, label: 'Exponent PT 🔒',  desc: 'Fixed-rate yield via Principal Tokens · ~22% APY · no impermanent loss' },
  { value: 6, label: 'JLP Hedge 🛡️',   desc: 'Delta-neutral: JLP long + CEX/hybrid short · ~35% APY · low directional risk' },
]
...
export function CreatePotModal({ open, onClose, onCreated }: CreatePotModalProps) {
  ...
  async function handleCreate() {
    ...
    try {
      const result = await createPot.mutateAsync({
        name: name.trim(), emoji, isPublic,
        minDeposit: parseFloat(minDeposit) || 0.01,
        lockupSeconds: 0, yieldStrategy,
        tradeLevel: govLevel,
        withdrawLevel: Math.min(govLevel + 1, 4),
        maxTradeSizeBps, maxMembers: 0, protocolFeeBps: 0,
      })
  ...
  function handleClose() {
    setStep(1); setName(''); setEmoji('🪴'); setIsPublic(true)
    setMinDeposit('0.1'); setYieldStrategy(0); setGovLevel(2)
    setMaxTradeSizeBps(2000); setPrivacyLevel('public'); setError(null)
    onClose()
  }
  ...
  return (
    <Modal open={open} onClose={handleClose} className="max-w-xl">
      <div className="flex items-center justify-between p-6 border-b border-[#1A2332]">
        ...
        <button onClick={handleClose} className="text-gray-400 hover:text-white transition-colors text-xl leading-none">×</button>
      </div>

      <div className="flex px-6 pt-4 gap-2">
        {Array.from({ length: TOTAL_STEPS }, (_, i) => i + 1).map(s => (
          <div key={s} className={`h-1 flex-1 rounded-full transition-colors ${s <= step ? 'bg-[#9945FF]' : 'bg-[#1A2332]'}`} />
        ))}
      </div>
      ...
      <div className="p-6 space-y-5">
          ...
          <>
            <div>
              ...
              <input
                ...
                placeholder="e.g. Alpha Fund, Degen Vault…" maxLength={32}
                ...
                {EMOJI_OPTIONS.map(e => (
                  <button key={e} onClick={() => setEmoji(e)}
                    className={`w-10 h-10 rounded-xl text-xl flex items-center justify-center transition-all ${
                      emoji === e ? 'bg-[#9945FF]/30 ring-2 ring-[#9945FF]' : 'bg-[#1A2332] hover:bg-[#243044]'
                    }`}>{e}</button>
                ))}
              </div>
            </div>
              ...
              <div className="flex items-center justify-between py-3 px-4">
                <div>
                  ...
                  <div className="text-xs text-gray-400 mt-0.5">
                    {isPublic ? 'Visible on the leaderboard — anyone can request to join' : 'Hidden from leaderboard — invite-only access'}
                  </div>
                </div>
                <button onClick={() => setIsPublic(!isPublic)}
                  className={`relative w-12 h-6 rounded-full transition-colors shrink-0 ${isPublic ? 'bg-[#9945FF]' : 'bg-[#1A2332]'}`}>
        ...
        {step === 2 && (
          <div className="space-y-2">
            <p className="text-sm text-gray-400 mb-4">Choose how idle SOL in the vault earns yield between trades.</p>
            {YIELD_OPTIONS.map(opt => (
              <button key={opt.value} onClick={() => setYieldStrategy(opt.value)}
                className={`w-full text-left px-4 py-3.5 rounded-xl border transition-all ${
                  yieldStrategy === opt.value ? 'border-[#9945FF] bg-[#9945FF]/10' : 'border-[#1A2332] bg-[#0D1117] hover:border-[#243044]'
                }`}>
        ...
        {step === 3 && (
          <div className="space-y-5">
            <div>
              <p className="text-sm text-gray-400 mb-3">Set the governance level for trade decisions.</p>
              <div className="space-y-2">
                {GOV_OPTIONS.map(opt => (
                  <button key={opt.value} onClick={() => setGovLevel(opt.value)}
                    className={`w-full text-left px-4 py-3 rounded-xl border transition-all ${
                      govLevel === opt.value ? 'border-[#9945FF] bg-[#9945FF]/10' : 'border-[#1A2332] bg-[#0D1117] hover:border-[#243044]'
                    }`}>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono bg-[#9945FF]/20 text-[#9945FF] px-1.5 py-0.5 rounded">L{opt.value}</span>
            ...
            <div>
              <p className="text-sm text-gray-400 mb-3 flex items-center gap-2"><span>🛡️</span><span>Max trade size per swap</span></p>
              <div className="grid grid-cols-5 gap-2">
                {TRADE_SIZE_OPTIONS.map(opt => (
                  <button key={opt.value} onClick={() => setMaxTradeSizeBps(opt.value)}
                    className={`text-center px-2 py-2.5 rounded-xl border transition-all ${
                      maxTradeSizeBps === opt.value ? 'border-[#9945FF] bg-[#9945FF]/10 text-white' : 'border-[#1A2332] bg-[#0D1117] text-gray-400 hover:border-[#243044]'
                    }`}>
              ...
              <p className="text-[10px] text-gray-600 mt-2">Limits any single proposed swap to this % of the vault balance</p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-5">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] bg-[#7C3AED]/20 text-[#7C3AED] px-1.5 py-0.5 rounded font-semibold">Powered by Umbra</span>
              </div>
              <p className="text-xs text-gray-400 mb-3">Umbra hides deposit amounts and trade sizes on-chain, protecting the vault from front-running and surveillance.</p>
              <div className="space-y-2">
                {PRIVACY_OPTIONS.map(opt => (
                  <button key={opt.value} onClick={() => setPrivacyLevel(opt.value)}
                    className={`w-full text-left px-4 py-3.5 rounded-xl border transition-all ${
                      privacyLevel === opt.value
                        ? opt.umbra ? 'border-[#7C3AED] bg-[#7C3AED]/10' : 'border-[#9945FF] bg-[#9945FF]/10'
                        : 'border-[#1A2332] bg-[#0D1117] hover:border-[#243044]'
                    }`}>
                    <div className="flex items-center gap-2">
                      <span>{opt.icon}</span>
                      <span className="font-semibold text-sm">{opt.label}</span>
                      {opt.umbra && <span className="text-[9px] bg-[#7C3AED]/20 text-[#7C3AED] px-1 py-0.5 rounded ml-auto">Umbra</span>}
                    </div>
                    <div className="text-xs text-gray-400 mt-0.5">{opt.desc}</div>
                  </button>
                ))}
            ...
            <div className="bg-[#0D1117] border border-[#9945FF]/20 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] bg-[#9945FF]/20 text-[#9945FF] px-1.5 py-0.5 rounded ml-auto">Powered by Kora</span>
              </div>
              ...
              <div className="flex items-center gap-4 mt-2.5">
                <div className="flex items-center gap-1 text-[10px] text-[#9945FF]"><span>✓</span><span>Deposit in USDC</span></div>
                <div className="flex items-center gap-1 text-[10px] text-[#9945FF]"><span>✓</span><span>Vote for free</span></div>
                <div className="flex items-center gap-1 text-[10px] text-[#9945FF]"><span>✓</span><span>No SOL required</span></div>
              </div>
            </div>
            ...
            {snsDomain && (
              <div className="bg-[#0D1117] border border-[#14F195]/20 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] bg-[#14F195]/10 text-[#14F195] px-1.5 py-0.5 rounded ml-auto">potbot.sol</span>
                </div>
                <div className="font-mono text-sm text-[#14F195] mb-1">{snsDomain}</div>
                <div className="text-[10px] text-pot-muted">Your vault will be accessible via this Solana Name Service subdomain.</div>
              </div>
            )}
            ...
            <div className="bg-[#0D1117] border border-[#1A2332] rounded-xl p-4 space-y-2">
              ...
              <SummaryRow label="Yield" value={YIELD_OPTIONS.find(o => o.value === yieldStrategy)?.label ?? 'None'} />
              <SummaryRow label="Governance" value={GOV_OPTIONS.find(o => o.value === govLevel)?.label ?? 'Majority'} />
              <SummaryRow label="Privacy" value={PRIVACY_OPTIONS.find(o => o.value === privacyLevel)?.label ?? 'Public'} />
              {snsDomain && <SummaryRow label="Domain" value={snsDomain} mono />}
              <SummaryRow label="Gas" value="⚡ Gasless · Kora" />
            </div>

            {error && <div className="text-xs text-red-400 bg-red-400/10 rounded-xl px-4 py-3">{error}</div>}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between p-6 border-t border-[#1A2332]">
        ...
        {step < TOTAL_STEPS ? (
          ...
          <button onClick={handleCreate} disabled={loading || !publicKey}
            className="btn-primary disabled:opacity-40 flex items-center gap-2">
            {loading ? <><Spinner size="sm" /> Creating…</> : <>🪴 Create POT <span className="text-[10px] opacity-60 font-normal">⚡ Gasless</span></>}
          </button>
        )}
      </div>
    </Modal>
