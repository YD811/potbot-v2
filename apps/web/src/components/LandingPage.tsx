...
const FEATURES = [
  { icon: '🏦', title: 'Group Vaults', desc: 'Pool SOL with friends into a shared on-chain vault. Every member gets proportional shares — like a crypto ETF.' },
  { icon: '🏗️', title: 'On-Chain Governance', desc: 'Every trade, withdrawal or strategy change requires a vote. Configure quorum, approval %, and voting windows.' },
  { icon: '🤖', title: 'AI Trading Agent', desc: 'Set IF/THEN rules — “if SOL drops 5%, buy 10%”. The MCP-native agent creates proposals and votes automatically.' },
  { icon: '🪙', title: 'Tokenized Shares', desc: 'Deposit SOL → receive SPL tokens at current NAV. Redeem anytime. Trade shares on-chain like a real ETF.' },
  { icon: '🐾', title: 'Tamagotchi Mascot', desc: 'Your vault grows from 🥚 Egg to ⚡ Titan. Higher levels unlock lower swap fees and exclusive benefits.' },
  { icon: '💰', title: 'Creator Economy', desc: 'Launch a Strategy Vault, set entry/performance fees, and earn from investors following your strategy.' },
]

const HOW_IT_WORKS = [
  { step: '01', title: 'Create a vault', desc: 'Choose a name, emoji, governance level (from autocracy to full consensus), and yield strategy. Takes 30 seconds.', color: 'text-pot-green', border: 'border-pot-green/30' },
  { step: '02', title: 'Members deposit', desc: 'Anyone sends SOL to the vault address — anonymously if needed. They receive shares proportional to the current NAV.', color: 'text-pot-accent', border: 'border-pot-accent/30' },
  { step: '03', title: 'Trade together', desc: 'Members propose swaps via Jupiter. AI agents create proposals automatically. Shares of vault determine voting power.', color: 'text-blue-400', border: 'border-blue-500/30' },
]
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          ...
          {totalTvlSol > 0 && (
            <div className="inline-flex flex-wrap items-center gap-0 bg-pot-card border border-pot-border rounded-2xl overflow-hidden shadow-xl">
              {[
                { label: 'Total Value Locked', value: totalTvlUsd > 0 ? `$${totalTvlUsd >= 1000 ? (totalTvlUsd / 1000).toFixed(1) + 'K' : totalTvlUsd.toFixed(0)}` : `${totalTvlSol.toFixed(1)} SOL`, color: 'text-pot-green' },
      ...
      {topVaults.length > 0 && (
        <section className="max-w-5xl mx-auto px-4 py-16">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/leaderboard" className="btn-secondary text-sm">Full Leaderboard →</Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {topVaults.map((pot, i) => {
              ...
              const medals = ['🥇', '🥈', '🥉']
              return (
                <Link key={pot.pubkey} href={`/pots/${pot.pubkey}`} className="card p-5 hover:border-pot-green/30 transition-all group">
                  ...
                  {balanceUsd > 0 && <div className="text-xs text-pot-muted">≈ ${balanceUsd >= 1000 ? (balanceUsd / 1000).toFixed(1) + 'K' : balanceUsd.toFixed(0)}</div>}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            {FOR_BUILDERS.map((item) => (
              <a key={item.title} href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel={item.href.startsWith('http') ? 'noreferrer' : undefined} className="card p-5 hover:border-pot-accent/30 transition-all group">
      ...
      <section className="max-w-3xl mx-auto text-center px-4 py-20">
        ...
        <h2 className="text-4xl font-black text-white mb-4">Ready to trade together?</h2>
        <p className="text-pot-muted text-lg mb-8">Create your vault in under a minute. No coding required. Open source and free to use on Solana devnet.</p>
        <div className="flex flex-wrap gap-4 justify-center">
          <Link href="/create" className="btn-primary text-lg px-8 py-4">🚀 Create Your Vault</Link>
          <a href="https://github.com/YD811/potbot-v2" target="_blank" rel="noreferrer" className="btn-secondary text-lg px-8 py-4">⭐ Star on GitHub</a>
        </div>
        <p className="text-pot-muted/50 text-xs mt-6">Demo mode active — no wallet needed to explore · Built on Solana devnet</p>
      </section>
