...
export default function SharesTab({ potPubkey }: Props) {
  ...
  const shareData = useMemo<ShareData | null>(() => {
    ...
    const userValueUsd = userBalance * navPerShare * 145 // mock SOL/USD

    // Deterministic fake mint from pubkey
    ...
    return {
      ...
      priceChange24h: ((Math.random() - 0.4) * 10),
    }
    ...
    <div className="space-y-6">
      ...
      <div className="card p-5 border border-pot-accent/30">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs px-2 py-0.5 rounded-full bg-pot-accent/20 text-pot-accent">
                SPL Token
              </span>
              <span className="text-xs px-2 py-0.5 rounded-full bg-pot-green/20 text-pot-green">
                ETF-Style
              </span>
            </div>
          ...
          <div className="ml-auto text-right">
            <div className="text-2xl font-bold text-white">
              {shareData.navPerShare.toFixed(4)} SOL
            </div>
        ...
        <div className="bg-pot-dark rounded-lg px-4 py-2 flex items-center gap-2">
          ...
          <span className="font-mono text-xs text-white truncate flex-1">
            {shareData.mintAddress}
          </span>
          <button
            onClick={() => navigator.clipboard.writeText(shareData.mintAddress)}
            className="text-xs text-pot-muted hover:text-white transition-colors flex-shrink-0"
          >
            copy
          </button>
        </div>
      ...
      {shareData.userBalance > 0 ? (
        <div className="card p-5 border border-pot-green/30">
          <h3 className="text-white font-semibold mb-4">📊 Моя позиция</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-pot-dark rounded-lg p-3">
              <div className="text-pot-muted text-xs mb-1">Баланс токенов</div>
              <div className="text-xl font-bold text-pot-green">
                {shareData.userBalance.toLocaleString()}
              </div>
              <div className="text-xs text-pot-muted">${shareData.symbol}</div>
            </div>
            <div className="bg-pot-dark rounded-lg p-3">
              <div className="text-pot-muted text-xs mb-1">Стоимость</div>
              <div className="text-xl font-bold text-white">
                {(shareData.userBalance * shareData.navPerShare).toFixed(3)} SOL
              </div>
              <div className="text-xs text-pot-muted">
                ≈ ${shareData.userValueUsd.toFixed(0)}
              </div>
            </div>
            <div className="bg-pot-dark rounded-lg p-3">
              <div className="text-pot-muted text-xs mb-1">Доля в пуле</div>
              <div className="text-xl font-bold text-pot-accent">
                {shareData.totalSupply > 0
                  ? ((shareData.userBalance / shareData.totalSupply) * 100).toFixed(2)
                  : '0.00'}%
              </div>
            </div>
            <div className="bg-pot-dark rounded-lg p-3">
              ...
              <div className="text-xl font-bold text-white">
                {shareData.navPerShare.toFixed(4)}
              </div>
          ...
          {/* Redeem button */}
          <button className="btn-secondary w-full mt-4 py-3 text-sm">
            🔥 Погасить токены (Redeem)
          </button>
        </div>
      ) : (
        <div className="card p-5 border border-pot-border text-center">
          <p className="text-4xl mb-2">🪙</p>
          <p className="text-white font-semibold mb-1">Нет токенов</p>
          <p className="text-pot-muted text-sm mb-4">
            Сделай депозит — и получишь ${shareData.symbol} пропорционально NAV
          </p>
      ...
      <div className="card p-5 border border-pot-border">
        <h3 className="text-white font-semibold mb-4">📈 Параметры фонда</h3>
        <div className="space-y-3">
          {[
            { label: 'AUM', value: `${shareData.aum.toFixed(2)} SOL`, sub: `≈ $${(shareData.aum * 145).toFixed(0)}` },
            { label: 'Total Supply', value: `${shareData.totalSupply.toLocaleString()} ${shareData.symbol}`, sub: 'токенов в обращении' },
            { label: 'NAV per share', value: `${shareData.navPerShare.toFixed(6)} SOL`, sub: 'Net Asset Value' },
            { label: 'Holders', value: `${shareData.holders}`, sub: 'участников' },
          ].map(({ label, value, sub }) => (
      ...
      <div className="card p-4 border border-pot-border">
        <h3 className="text-pot-green font-semibold mb-3 text-sm">💡 Как работают токены</h3>
        <div className="space-y-2 text-xs text-pot-muted">
          <p>• <span className="text-white">Mint:</span> При депозите SOL → получаешь ${shareData.symbol} по текущему NAV</p>
          <p>• <span className="text-white">Burn:</span> При выводе ${shareData.symbol} → получаешь SOL по текущему NAV</p>
          <p>• <span className="text-white">NAV</span> = (общий баланс пота) / (total supply) — пересчитывается при каждом свапе</p>
          <p>• Токены можно торговать на DEX — цена следует за NAV вота</p>
          <p>• Аналог ETF: владеешь токеном = владеешь долей всех активов пота</p>
        </div>
      </div>
    </div>
  )
