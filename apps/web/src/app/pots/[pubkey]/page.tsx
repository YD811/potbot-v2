      ...
      <div className="card p-6 mb-6">
              ...
              <div>
                ...
                {snsName ? (
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="text-xs font-mono text-pot-green">{snsName}</span>
                    <button
                      onClick={() => {
                        const text = getPotShareText(pot.name, snsName)
                        navigator.clipboard.writeText(text)
                          .catch(() => {})
                      }}
                      className="text-[10px] text-pot-muted hover:text-pot-green transition px-1 py-0.5 rounded bg-pot-dark"
        ...
        <div className="flex gap-2 mt-4">
          ...
          <span className="text-xs px-2 py-0.5 rounded-full bg-pot-border text-pot-muted">
            {({ 0: 'No Yield', 1: '📈 Conservative', 2: '⚖️ Balanced', 3: '🔥 Aggressive', 4: '⚡ JLP' } as Record<number,string>)[pot.yieldStrategy] ?? 'Yield'}
          </span>
          <span className="text-xs px-2 py-0.5 rounded-full bg-pot-border text-pot-muted">
          ...
          {snsName && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-pot-green/10 text-pot-green border border-pot-green/20 font-mono">
              🌐 {snsName}
            </span>
          )}
          {isAdmin && (
...
// ── Numeric keys matching mock-store.ts YIELD_APY / YIELD_RESERVE_PCT ────────
...
const YIELD_PROTOCOLS: Record<number, string> = {
  0: '—',
  1: 'Kamino stablecoins',
  2: 'Mixed lending',
  3: 'High-risk farms',
  4: 'Jupiter LP · delta-hedged via Drift',
}

function YieldPanel({ pot }: { pot: any }) {
  const strategy: number = typeof pot.yieldStrategy === 'number' ? pot.yieldStrategy : 0
  const apy = YIELD_APY[strategy] ?? 0
  const reservePct = YIELD_RESERVE_PCT[strategy] ?? 0
  const isJLP = strategy === 4
  ...
  const strategyColor = strategyColors[strategy] ?? 'text-pot-muted'
  const barColor = isJLP ? 'bg-yellow-500' : 'bg-teal-500'
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-pot-muted mt-0.5">{YIELD_PROTOCOLS[strategy]}</p>
        </div>
        <div className="text-right">
          <div className={`text-lg font-bold font-mono ${strategyColor}`}>
            {(apy * 100).toFixed(0)}% APY
          </div>
          <div className={`text-xs font-semibold ${strategyColor}`}>{YIELD_LABELS[strategy]}</div>
        </div>
      </div>
      ...
      {isJLP && (
        <div className="mb-4 rounded-xl border border-yellow-500/30 bg-yellow-500/10 p-3">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-semibold text-yellow-300 mb-0.5">Hedged Jupiter LP (hJLP)</p>
              <p className="text-xs text-yellow-200/70">
                Earns Jupiter JLP fees (30–60% APY) while delta-hedging directional risk via Drift Protocol shorts.
                Net portfolio delta ≈ 0 — yield without directional exposure.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-5">
        <div className="flex justify-between text-xs text-pot-muted mb-1.5">
          ...
          <span>{isJLP ? 'In JLP' : 'In Meteora'} <span className={`${strategyColor} font-mono`}>{meteoraPct.toFixed(0)}%</span></span>
        </div>
        <div className="flex h-3 rounded-full overflow-hidden gap-0.5">
          <div className="bg-purple-500 h-full rounded-l-full transition-all" style={{ width: `${liquidPct}%` }} />
          {meteoraPct > 0 && (
            <div className={`${barColor} h-full rounded-r-full transition-all`} style={{ width: `${meteoraPct}%` }} />
          )}
        </div>
        <div className="flex justify-between text-xs text-pot-muted mt-1">
          <span className="font-mono">{liquidBal.toFixed(4)} SOL</span>
          <span className={`font-mono ${strategyColor}`}>{meteoraBal.toFixed(4)} SOL {isJLP ? 'in JLP' : 'equivalent'}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-pot-dark rounded-xl p-3 text-center">
          ...
          <div className={`text-sm font-mono font-semibold ${strategyColor}`}>
        ...
        <div className="bg-pot-dark rounded-xl p-3 text-center">
          <div className="text-xs text-pot-muted mb-1">In Protocol</div>
      ...
      {/* Delta hedge stat (JLP only) */}
      {isJLP && (
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div className="bg-pot-dark rounded-xl p-3 text-center">
            <div className="text-xs text-pot-muted mb-1">Portfolio Delta</div>
            <div className="text-sm font-mono font-semibold text-yellow-400">Δ ≈ 0</div>
          </div>
          <div className="bg-pot-dark rounded-xl p-3 text-center">
            <div className="text-xs text-pot-muted mb-1">Hedge Protocol</div>
            <div className="text-sm font-mono font-semibold text-white">Drift</div>
          </div>
        </div>
      )}
      ...
      {apy > 0 && (
        <div className="mt-4 flex items-center gap-2 text-xs text-pot-muted">
          <span className="relative flex h-2 w-2">
            <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${isJLP ? 'bg-yellow-400' : 'bg-teal-400'} opacity-75`} />
            <span className={`relative inline-flex rounded-full h-2 w-2 ${isJLP ? 'bg-yellow-500' : 'bg-teal-500'}`} />
          </span>
          Live accrual — {isJLP ? 'JLP fees + funding rate accrual every 10 seconds' : 'yield updates every 10 seconds from Meteora simulation'}
        </div>
      )}
    </div>
  )
...
function GovernancePanel({
  ...
  const govSubTabs = [
    ...
    ...(pot.yieldStrategy !== 0
      ? [{ key: 'yield', label: pot.yieldStrategy === 4 ? '⚡ JLP Yield' : '🌱 Yield' }]
      : []),
...
function ProposalCard({
  ...
  const isLimitOrder   = proposal.type === 'limitOrder'
  const isDCA          = proposal.type === 'dca'
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1 min-w-0 pr-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              {isLimitOrder && (
                <span className="text-[10px] bg-[#14F195]/10 text-[#14F195] px-1.5 rounded-full shrink-0 border border-[#14F195]/20">
                  ⚡ Limit Order · Jupiter
                </span>
              )}
              {isDCA && (
                <span className="text-[10px] bg-blue-500/10 text-blue-400 px-1.5 rounded-full shrink-0 border border-blue-500/20">
                  🔄 DCA · Jupiter
                </span>
              )}
            </div>
        ...
        {connected && proposal.status === 'active' && (
          <div className="space-y-1.5">
            ...
            <div className="flex items-center justify-center gap-1 text-[10px] text-[#9945FF]/60">
              <span>⚡</span>
              <span>Gasless · Powered by Kora · no SOL needed</span>
            </div>
          </div>
        )}
