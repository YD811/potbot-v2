...
import type { DbPot, DbMember, DbProposal, DbVote, DbReferral, DbAgentRule, DbGovernanceSettings } from './supabase'

// ─── Pots ────────────────────────────────────────────────────────────────────
export async function getPots(): Promise<DbPot[]> {
  if (!isSupabaseConfigured) return []
  const { data, error } = await supabase.from('pots').select('*').order('balance', { ascending: false })
  if (error) throw error
  return data ?? []
}
export async function getPot(pubkey: string): Promise<DbPot | null> {
  if (!isSupabaseConfigured) return null
  const { data } = await supabase.from('pots').select('*').eq('pubkey', pubkey).single()
  return data ?? null
}
export async function upsertPot(pot: Partial<DbPot> & { pubkey: string }) {
  const db = createServerSupabase()
  const { error } = await db.from('pots').upsert({ ...pot, updated_at: new Date().toISOString() })
  if (error) throw error
}
...
export async function getMembers(potPubkey: string): Promise<DbMember[]> {
  if (!isSupabaseConfigured) return []
  const { data, error } = await supabase.from('members').select('*').eq('pot_pubkey', potPubkey)
  if (error) throw error
  return data ?? []
}
export async function getMember(potPubkey: string, wallet: string): Promise<DbMember | null> {
  if (!isSupabaseConfigured) return null
  const { data } = await supabase.from('members').select('*').eq('pot_pubkey', potPubkey).eq('wallet', wallet).single()
  return data ?? null
}
...
export async function getProposals(potPubkey: string): Promise<DbProposal[]> {
  if (!isSupabaseConfigured) return []
  const { data, error } = await supabase.from('proposals').select('*').eq('pot_pubkey', potPubkey).order('created_at', { ascending: false })
  if (error) throw error
  return data ?? []
}
...
export async function createProposal(proposal: Omit<DbProposal, 'created_at' | 'executed_at'>) {
  ...
  if (error) throw error
  await db.from('pots').update({ next_proposal_id: proposal.proposal_id + 1, updated_at: new Date().toISOString() }).eq('pubkey', proposal.pot_pubkey)
  return data
}
...
export async function getVotes(proposalPubkey: string): Promise<DbVote[]> {
  if (!isSupabaseConfigured) return []
  const { data, error } = await supabase.from('votes').select('*').eq('proposal_pubkey', proposalPubkey)
  if (error) throw error
  return data ?? []
}
...
export async function getReferrals(potPubkey: string, wallet: string): Promise<DbReferral[]> {
  if (!isSupabaseConfigured) return []
  const { data, error } = await supabase.from('referrals').select('*').eq('pot_pubkey', potPubkey).or(`referrer.eq.${wallet},parent_referrer.eq.${wallet}`)
  if (error) throw error
  return data ?? []
}
...
export async function getAgentRules(potPubkey: string): Promise<DbAgentRule[]> {
  if (!isSupabaseConfigured) return []
  const { data, error } = await supabase.from('agent_rules').select('*').eq('pot_pubkey', potPubkey).eq('enabled', true)
  if (error) throw error
  return data ?? []
}
export async function getAllEnabledRules(): Promise<DbAgentRule[]> {
  const db = createServerSupabase()
  const { data, error } = await db.from('agent_rules').select('*').eq('enabled', true)
  if (error) throw error
  return data ?? []
}
...
export async function markRuleTriggered(potPubkey: string, ruleId: string) {
  const db = createServerSupabase()
  await db.from('agent_rules').update({ last_triggered_at: new Date().toISOString() }).eq('pot_pubkey', potPubkey).eq('rule_id', ruleId)
}

// ─── Governance Settings ─────────────────────────────────────────────────────
export async function getGovernanceSettings(potPubkey: string): Promise<DbGovernanceSettings | null> {
  if (!isSupabaseConfigured) return null
  const { data } = await supabase.from('governance_settings').select('*').eq('pot_pubkey', potPubkey).single()
  return data ?? null
}
export async function upsertGovernanceSettings(settings: DbGovernanceSettings) {
  const db = createServerSupabase()
  const { error } = await db.from('governance_settings').upsert({ ...settings, updated_at: new Date().toISOString() })
  if (error) throw error
}
...
export async function getLatestPrice(token = 'SOL'): Promise<number | null> {
  if (!isSupabaseConfigured) return null
  const { data } = await supabase.from('price_history').select('price').eq('token', token).order('recorded_at', { ascending: false }).limit(1).single()
