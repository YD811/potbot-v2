...
use crate::state::pot::{PotAccount, PotConfig, GovSettings, YieldStrategy};
...
pub struct CreatePrivatePotParams {
    pub name: String,
    pub emoji: String,
    pub invite_code: String, // 6-character alphanumeric code
    pub config: PotConfig,
    pub governance: GovSettings,
}

#[derive(Accounts)]
#[instruction(params: CreatePrivatePotParams)]
pub struct CreatePrivatePot<'info> {
    #[account(
        init,
        payer = authority,
        space = 8 + PotAccount::INIT_SPACE,
        seeds = [b"pot", params.name.as_bytes(), authority.key().as_ref()],
        bump
    )]
...
pub fn handler(ctx: Context<CreatePrivatePot>, params: CreatePrivatePotParams) -> Result<()> {
    let pot = &mut ctx.accounts.pot;
    let private_pot = &mut ctx.accounts.private_pot_account;
    let now = Clock::get()?.unix_timestamp;
    
    // Validate invite code (6 chars, alphanumeric)
    require!(
    ...
    // Validate pot name
    require!(
        !params.name.trim().is_empty() && params.name.len() <= 32,
        ErrorCode::InvalidPotName
    );
    
    // Initialize POT account (same as regular pot)
    ...
    pot.protocol_fee_bps = 250; // 2.5% default
    pot.last_activity_at = now;
    pot.agent_pubkey = None;
    pot.agent_max_trade_bps = 1000; // 10% default
    ...
    // CRITICAL: Mark as private by setting is_public = false
    pot.config.is_public = false;
    
    // Initialize private pot account
    private_pot.pot = pot.key();
    private_pot.invite_code = params.invite_code.to_uppercase(); // Store in uppercase
    private_pot.created_at = now;
    private_pot.invited_members = Vec::new();
    private_pot.max_invites = 100; // Default limit
